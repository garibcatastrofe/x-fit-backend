...
export class UpdateRutina {
  public constructor(private readonly rutinaRepo: RutinaRepository) {}

  public async run(id: string, rutina: RutinaPrimitive): Promise<void> {
    const rutinaId = new RutinaId(id);

    const b1_sesion1 = (await Promise.all(
      rutina.bloques.bloque_1.sesion_1.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b1_sesion2 = (await Promise.all(
      rutina.bloques.bloque_1.sesion_2.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b1_sesion3 = (await Promise.all(
      rutina.bloques.bloque_1.sesion_3.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b1_sesion4 = (await Promise.all(
      rutina.bloques.bloque_1.sesion_4.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b1_sesion5 = (await Promise.all(
      rutina.bloques.bloque_1.sesion_5.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b1_sesion6 = (await Promise.all(
      rutina.bloques.bloque_1.sesion_6.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];

    const b2_sesion1 = (await Promise.all(
      rutina.bloques.bloque_2.sesion_1.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b2_sesion2 = (await Promise.all(
      rutina.bloques.bloque_2.sesion_2.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b2_sesion3 = (await Promise.all(
      rutina.bloques.bloque_2.sesion_3.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b2_sesion4 = (await Promise.all(
      rutina.bloques.bloque_2.sesion_4.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b2_sesion5 = (await Promise.all(
      rutina.bloques.bloque_2.sesion_5.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b2_sesion6 = (await Promise.all(
      rutina.bloques.bloque_2.sesion_6.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];

    const b3_sesion1 = (await Promise.all(
      rutina.bloques.bloque_3.sesion_1.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b3_sesion2 = (await Promise.all(
      rutina.bloques.bloque_3.sesion_2.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b3_sesion3 = (await Promise.all(
      rutina.bloques.bloque_3.sesion_3.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b3_sesion4 = (await Promise.all(
      rutina.bloques.bloque_3.sesion_4.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b3_sesion5 = (await Promise.all(
      rutina.bloques.bloque_3.sesion_5.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];
    const b3_sesion6 = (await Promise.all(
      rutina.bloques.bloque_3.sesion_6.map(a => Ejercicio.create(a.id_ejercicio)),
    )) as Ejercicio[];

    const nuevaRutina = new Rutina(
      id ? new RutinaId(id) : RutinaId.retornoVacio(),
      new ClienteId(rutina.id_cliente),
      new EmpleadoId(rutina.id_empleado),
      new RutinaFechaCreacion(rutina.fecha_creacion),
      new RutinaObjetivo(rutina.objetivo),
      new Visible(rutina.visible),
      new BloquesRutina(
      ...
      new DatosEspecificosGrupoMuscularRutina(
        new ObjetivoPrograma(rutina.datos_especificos_grupo_muscular.objetivo_programa),
        rutina.datos_especificos_grupo_muscular.grupo_muscular.map(
          grupo =>
            new DatoEspecifico(
    ...
    await this.rutinaRepo.update(rutinaId.value, nuevaRutina.toRutinaPrimitive());
  }
}
